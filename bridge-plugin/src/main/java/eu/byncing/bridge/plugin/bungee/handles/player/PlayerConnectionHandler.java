...
public class PlayerConnectionHandler implements IPacketHandler<Packet> {
    ...
    public void handle(IChannel channel, Packet packet) {
        if (packet instanceof PacketPlayerConnect) {
            ...
            BridgeService service = (BridgeService) server.getServices().getService(connect.getServiceUniqueId());
            service.getPlayers().add(player);
            player.setService(service);
            server.sendPacket(new PacketPlayerConnect(player.getUniqueId(), player.getName(), service.getUniqueId()));
        ...
        if (packet instanceof PacketPlayerDisconnect) {
            ...
            BridgeService service = (BridgeService) server.getServices().getService(disconnect.getServiceUniqueId());
            ...
            server.sendPacket(new PacketPlayerDisconnect(player.getUniqueId(), player.getName(), service.getUniqueId()));
        }
    }
